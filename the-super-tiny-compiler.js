 ...
 * and Code Generation 解析/转换/代码生成
 *
 ...
 * Syntactic Analysis. 词法分析/语法分析
 *
 * 1. *Lexical Analysis* takes the raw code and splits it apart into these things
 *    called tokens by a thing called a tokenizer (or lexer).
 *    将原始代码分解成 tokens （通过叫做 tokenizer 或是 lexer 的东西来进行分解的）
 *
 *    Tokens are an array of tiny little objects that describe an isolated piece
 *    of the syntax. They could be numbers, labels, punctuation 标点符号, operators,
 *    whatever.
 *    Tokens 是一组描述语法的独立部分的小对象
 *
 * 2. *Syntactic Analysis* takes the tokens and reformats them into a
 *    representation that describes each part of the syntax and their relation
 *    to one another. This is known as an intermediate representation or
 *    Abstract Syntax Tree.
 *    语法分析 将上面的Tokens 进行重新格式化，表示成 描述语法各个部分及相互关系
 *    抽象语法数
 *
 *    An Abstract Syntax Tree, or AST for short, is a deeply nested object that
 *    represents code in a way that is both easy to work with and tells us a lot
 *    of information.
 *    是一个深度嵌套的对象，这种解释代码的方式 可以方便使用 同时 展示给我们很多信息
 *
 ...
 * takes the AST from the last step and makes changes to it. It can manipulate操作
 * the AST in the same language or it can translate it into an entirely new
 * language. 操作上一步的AST ，将他转化为同一语言或另一种语言
 *
 * Let’s look at how we would transform an AST.
 *
 * You might notice that our AST has elements within it that look very similar.结构上的相似
 * There are these objects with a type property. Each of these are known as an
 * AST Node 节点. These nodes have defined properties on them that describe one
 * isolated part of the tree. 这些节点定义了这个抽象语法树描述一个独立部分的属性
 *
 ...
 *   }
 *
 * When transforming the AST we can manipulate nodes by 转化AST 我们 可以 操作节点 是 通过添加/移除/替换这些属性来完成的
 * adding/removing/replacing properties, we can add new nodes, remove nodes, or
 * we could leave the existing AST alone and create an entirely new one based
 * on it. 我们可以添加新的节点、移除节点 或者我们可以 保留现有的AST 然后创建一个基于它的新的AST
 *
 * Since we’re targeting a new language, we’re going to focus on creating an
 * entirely new AST that is specific to the target language.
 * 因为我们的目标是一门新的语言，我们将关注与创建一个针对目标语言的新的AST
 *
 * Traversal遍历
 * ---------
 *
 * In order to navigate through all of these nodes, we need to be able to
 * 为了在所有这些节点中导航，我们需要能够遍历它们。
 * traverse through them. This traversal process goes to each node in the AST
 * depth-first.遍历AST中的每个节点 通过深度优先
 *
 *   {
 *     type: 'Program',
 *     body: [{
